...
pub fn filter_by_polygon_batch(points: &[(f32, f32)], polygon: &[(f32, f32)]) -> Result<Vec<bool>> {
    Ok(points
        .par_iter()
...
/// Batch point-in-range query (1D — only x coordinate checked)
pub fn filter_by_range_batch(
    points: &[(f32, f32)],
    bounds: (f32, f32),
) -> Result<Vec<bool>> {
...
mod tests {
    ...
    fn test_range_filtering() {
        ...
        let results = filter_by_range_batch(&points, bounds).unwrap();
    ...
    fn test_ellipse_filtering() {
        ...
        let results = filter_by_ellipse_batch(&points, center, radius_x, radius_y, angle).unwrap();

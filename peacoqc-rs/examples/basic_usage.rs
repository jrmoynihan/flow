use peacoqc_rs::{DoubletConfig, MarginConfig, PeacoQCData, Result, fcs::SimpleFcs, qc::*};
...
fn create_synthetic_data() -> Result<SimpleFcs> {
    ...
    let fsc_a: Vec<f64> = (0..n_events)
        .map(|_| 50000.0 + rng.random::<f64>() * 20000.0)
        .collect();

    // FSC-H: Correlated with FSC-A (singlets)
    let fsc_h: Vec<f64> = fsc_a
        .iter()
        .map(|&a| a * 0.8 + rng.random::<f64>() * 5000.0)
        .collect();

    // SSC-A: Normal distribution
    let ssc_a: Vec<f64> = (0..n_events)
        .map(|_| 30000.0 + rng.random::<f64>() * 15000.0)
        .collect();

    // FL1-A: Bimodal distribution (two populations)
    let mut fl1_a = Vec::new();
    for _ in 0..n_events {
        let val = if rng.random::<f64>() < 0.6 {
            // Population 1: low expression
            1000.0 + rng.random::<f64>() * 500.0
        } else {
            // Population 2: high expression
            5000.0 + rng.random::<f64>() * 1000.0
        };
        fl1_a.push(val);
    }
    ...
    let fl2_a: Vec<f64> = (0..n_events)
        .map(|_| 2000.0 + rng.random::<f64>() * 800.0)
        .collect();

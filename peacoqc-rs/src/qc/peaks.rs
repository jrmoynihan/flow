...
pub fn determine_peaks_all_channels<T: PeacoQCData>(
    ...
    let n_bins = (n_events + config.events_per_bin - 1) / config.events_per_bin;
    ...
    for channel in channels {
        let peak_frame = determine_channel_peaks(fcs, channel, &breaks, config)?;
        if let Some(frame) = peak_frame {
            results.insert(channel.clone(), frame);
        }
    }
...
fn create_breaks(n_events: usize, events_per_bin: usize) -> Vec<(usize, usize)> {
    let n_bins = (n_events + events_per_bin - 1) / events_per_bin;
    
    (0..n_bins)
        .map(|i| {
            let start = i * events_per_bin;
            let end = ((i + 1) * events_per_bin).min(n_events);
            (start, end)
        })
        .collect()
}
...
fn determine_channel_peaks<T: PeacoQCData>(
    fcs: &T,
    channel: &str,
    breaks: &[(usize, usize)],
    config: &PeakDetectionConfig,
    ...
    let series = df.column(channel)
        .map_err(|_| PeacoQCError::ChannelNotFound(channel.to_string()))?;
    
    let values = series.f64()
        .map_err(|_| PeacoQCError::InvalidChannel(channel.to_string()))?;
    ...
    let mut bin_peaks: Vec<Vec<f64>> = Vec::new();
    ...
    for (bin_idx, &(start, end)) in breaks.iter().enumerate() {
        ...
        if bin_data.len() < 3 {
            bin_peaks.push(Vec::new());
            continue;
        }
        
        // Compute KDE and find peaks
        let kde = KernelDensity::estimate(&bin_data, 1.0, 512)?;
        let peaks = kde.find_peaks(config.peak_removal);
        
        bin_peaks.push(peaks);
    }
    ...
    cluster_peaks(&mut all_peaks, &bin_peaks, config)?;
    
    // Remove small clusters
    remove_small_clusters(&mut all_peaks, breaks.len())?;
...
fn cluster_peaks(
    ...
    let min_bins = (config.min_nr_bins_peakdetection / 100.0 * peak_counts.len() as f64).ceil() as usize;
    
    let most_common_count = count_freq.iter()
    ...
    if reference_peaks.is_empty() {
        // Just use median of all peaks
        let all_values: Vec<f64> = all_peaks.iter().map(|p| p.peak_value).collect();
        let cluster_median = median(&all_values)?;
    ...
    for cluster_idx in 0..n_clusters {
        let values: Vec<f64> = reference_peaks.iter()
...
fn remove_small_clusters(all_peaks: &mut Vec<PeakInfo>, n_bins: usize) -> Result<()> {
    ...
    for peak in all_peaks.iter() {
        cluster_bin_counts.entry(peak.cluster)
    ...
    let clusters_to_keep: Vec<usize> = cluster_bin_counts.iter()
...
mod tests {
    ...
    fn test_peak_detection_basic() {
        ...
        let result = determine_peaks_all_channels(
            &fcs,
            &["FL1-A".to_string()],
            &config,
        );

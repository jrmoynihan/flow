...
use plotters::style::{RGBColor, RGBAColor, BLACK, WHITE};
...
fn calculate_events_per_second<T: PeacoQCData>(
    ...
    let time_series = df.column(time_channel)
        .map_err(|_| PeacoQCError::ChannelNotFound(time_channel.to_string()))?;
    
    let time_values: Vec<f64> = time_series.f64()
        .map_err(|_| PeacoQCError::InvalidChannel(time_channel.to_string()))?
        .into_iter()
        .filter_map(|x| x)
        .collect();
...
pub fn create_qc_plots<T: PeacoQCData>(
    ...
    let total_cells = config.n_cols * config.n_rows;
    
    if n_plots > total_cells {
        return Err(PeacoQCError::ConfigError(format!(
            "Too many plots ({}) for grid size ({}x{}={})",
            n_plots, config.n_rows, config.n_cols, total_cells
        )));
    }
    ...
    let plot_width = (config.width as f64 / config.n_cols as f64) as u32;
    let plot_height = (config.height as f64 / config.n_rows as f64) as u32;
    ...
    // Plot 1: Time vs events/second
    {
        ...
        if !events_per_sec.is_empty() {
            let x_range = events_per_sec.iter()
                .map(|(t, _)| *t)
                .fold((f64::INFINITY, f64::NEG_INFINITY), |(min, max), x| {
                    (min.min(x), max.max(x))
                });
            
            let y_range = events_per_sec.iter()
                .map(|(_, r)| *r)
                .fold((f64::INFINITY, f64::NEG_INFINITY), |(min, max), x| {
                    (min.min(x), max.max(x))
                });
            ...
            let subplot_area = root.split_evenly((config.n_rows, config.n_cols))[0];
            ...
            let text = format!("{:.3}% of the data was removed.", qc_result.percentage_removed);
    ...
    for (plot_idx, channel) in channels.iter().enumerate() {
        ...
        let y_max = channel_data.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
        ...
        let subplot_area = root.split_evenly((config.n_rows, config.n_cols))[subplot_idx];
        
        // Calculate MAD percentage for title
        let mad_pct = qc_result.mad_percentage
            .and_then(|_| {
        ...
        if !good_points.is_empty() {
            chart.draw_series(
                good_points.iter().map(|(x, y)| Circle::new((*x, *y), 1, config.good_color.filled())),
            )?;
        }
        ...
        if !medians.is_empty() {
            let median_points: Vec<(f64, f64)> = medians.iter()
                .map(|(bin_idx, median)| {

...
use peacoqc_rs::qc::isolation_tree::{IsolationTreeConfig, build_feature_matrix};
...
fn test_feature_matrix_structure_per_cluster() {
    ...
    assert_eq!(
        matrix[0].len(),
        4,
        "Feature matrix should have 4 columns (2 channels × 2 clusters)"
    );
    ...
    assert!(
        feature_names
            .iter()
            .any(|n| n.contains("FL1-A") && n.contains("cluster_1"))
    );
    assert!(
        feature_names
            .iter()
            .any(|n| n.contains("FL1-A") && n.contains("cluster_2"))
    );
    assert!(
        feature_names
            .iter()
            .any(|n| n.contains("FL2-A") && n.contains("cluster_1"))
    );
    assert!(
        feature_names
            .iter()
            .any(|n| n.contains("FL2-A") && n.contains("cluster_2"))
    );
...
fn test_feature_matrix_cluster_median_defaults() {
    ...
    for bin_idx in 0..5 {
        assert!(
            matrix[bin_idx][0].is_finite(),
            "Bin {} should have a value",
            bin_idx
        );
    }
    ...
    assert!(
        (matrix[0][0] - 100.0).abs() < 0.01,
        "Bin 0 should have peak value 100.0"
    );
    assert!(
        (matrix[2][0] - 120.0).abs() < 0.01,
        "Bin 2 should have peak value 120.0"
    );
    assert!(
        (matrix[4][0] - 110.0).abs() < 0.01,
        "Bin 4 should have peak value 110.0"
    );

    // Bins without peaks should have cluster median (110.0)
    assert!(
        (matrix[1][0] - 110.0).abs() < 0.01,
        "Bin 1 should have cluster median 110.0"
    );
    assert!(
        (matrix[3][0] - 110.0).abs() < 0.01,
        "Bin 3 should have cluster median 110.0"
    );
}
...
fn test_isolation_tree_feature_matrix() {
    ...
    assert!(
        result.is_ok(),
        "IT should succeed with correct feature matrix"
    );

    let result = result.unwrap();
    assert_eq!(
        result.outlier_bins.len(),
        20,
        "Should have 20 bins in result"
    );
}
...
fn test_mad_filters_to_it_passed_bins() {
    ...
    assert_eq!(
        result.outlier_bins.len(),
        10,
        "MAD result should have one entry per bin"
    );
}
...
fn test_preprocessing_order() {
    ...
    assert!(
        true,
        "Preprocessing order: RemoveMargins → RemoveDoublets → Compensate → Transform"
    );
}
...
fn test_doublet_removal_scaled_mad() {
    ...
    assert!(
        (mad_scaled - mad_raw * MAD_SCALE_FACTOR).abs() < 1e-10,
    ...
    assert!(
        (median_scaled - median_raw).abs() < 1e-10,
...
fn test_isolation_tree_finds_largest_group() {
    ...
    assert!(
        largest_node_size >= 10,
        "Largest node should have at least 10 bins, got {}",
        largest_node_size
    );
}
...
fn test_feature_matrix_missing_peaks() {
    ...
    for bin_idx in [1, 2, 3, 4, 6, 7, 8] {
        assert!(
            (matrix[bin_idx][0] - 110.0).abs() < 1.0,
            "Bin {} should have cluster median ~110.0, got {}",
            bin_idx,
            matrix[bin_idx][0]
        );
    }
...
fn test_feature_matrix_multiple_channels_clusters() {
    ...
    let has_fl1_c1 = feature_names
        .iter()
        .any(|n| n.contains("FL1-A") && n.contains("cluster_1"));
    let has_fl1_c2 = feature_names
        .iter()
        .any(|n| n.contains("FL1-A") && n.contains("cluster_2"));
    let has_fl2_c1 = feature_names
        .iter()
        .any(|n| n.contains("FL2-A") && n.contains("cluster_1"));
    let has_fl2_c2 = feature_names
        .iter()
        .any(|n| n.contains("FL2-A") && n.contains("cluster_2"));
    let has_fl2_c3 = feature_names
        .iter()
        .any(|n| n.contains("FL2-A") && n.contains("cluster_3"));

    assert!(
        has_fl1_c1 && has_fl1_c2 && has_fl2_c1 && has_fl2_c2 && has_fl2_c3,
        "All expected clusters should be present in feature names: {:?}",
        feature_names
    );
}
...
fn test_consecutive_bins_filtering() {
    ...
    assert!(
        ...
        "Short good region (3 bins) not at edge should be removed. Filtered: {:?}",
        filtered
    );
    ...
    assert!(
        ...
        "Short good region (2 bins) not at edge should be removed. Filtered: {:?}",
        filtered
    );
    ...
    assert!(
        !filtered[9] && !filtered[10] && !filtered[11] && !filtered[12] && !filtered[13],
        "Long good region (5 bins) should be kept. Filtered: {:?}",
        filtered
    );
}

...
fn bench_kde_cpu_vs_gpu(c: &mut Criterion) {
    ...
    for size in [50_000, 100_000, 500_000, 1_000_000, 2_000_000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("CPU", size), &data, |b, data| {
            b.iter(|| KernelDensity::estimate(black_box(data), 1.0, 512).unwrap())
        });
...
fn bench_kde_batched_gpu(c: &mut Criterion) {
    #[cfg(feature = "gpu")]
    {
        use peacoqc_rs::gpu::{GpuContext, KdeContext, is_gpu_available, kde_fft_batched_gpu};
        ...
        for n_channels in [5, 10].iter() {
            for size_per_channel in [50_000, 100_000, 500_000, 1_000_000].iter() {
                ...
                group.bench_with_input(
                    BenchmarkId::new(
                        "Batched GPU",
                        format!("{}ch_{}events", n_channels, size_per_channel),
                    ),
                    &(
                        all_data.clone(),
                        all_grids.clone(),
                        all_bandwidths.clone(),
                        all_ns.clone(),
                    ),
                    |b, (all_data, all_grids, all_bandwidths, all_ns)| {
                        let mut gpu_ctx = GpuContext::new().unwrap();
                        b.iter(|| {
                            let contexts: Vec<KdeContext> = all_data
                                .iter()
                                .zip(all_grids.iter())
...
fn bench_feature_matrix_cpu_vs_gpu(c: &mut Criterion) {
    ...
    .iter()
    {
        ...
        group.bench_with_input(
            ...
            &peak_results,
            |b, pr| b.iter(|| build_feature_matrix(black_box(pr), *n_bins).unwrap()),
        );
        ...
        #[cfg(feature = "gpu")]
        {
            ...
            if is_gpu_available() {
                group.bench_with_input(
                    ...
                    &peak_results,
                    |b, pr| b.iter(|| build_feature_matrix_gpu(black_box(pr), *n_bins).unwrap()),
                );
            }
        }
    }
...
criterion_group!(
    benches,
    bench_kde_cpu_vs_gpu,
    bench_kde_batched_gpu,
    bench_feature_matrix_cpu_vs_gpu
);

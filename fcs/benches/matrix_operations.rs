...
fn generate_compensation_matrix(n: usize) -> Mat<f32> {
    use rand::Rng;
...
fn bench_matrix_inversion(c: &mut Criterion) {
    ...
    for &n in &sizes {
        ...
        group.bench_with_input(BenchmarkId::new("CPU_LAPACK", n), &matrix, |b, m| {
            b.iter(|| black_box(MatrixOps::invert_matrix(m.as_ref())).unwrap());
        });
    }
...
fn bench_batch_matvec_cpu(c: &mut Criterion) {
    ...
    for &(n_channels, n_events) in &test_cases {
        ...
        group.bench_with_input(
            ...
            &(&matrix, &channel_data),
            |b, (m, d)| {
                b.iter(|| black_box(MatrixOps::batch_matvec(m.as_ref(), *d)).unwrap());
            },
        );
    }

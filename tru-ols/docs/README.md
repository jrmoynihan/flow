...
| [PROFILING.md](PROFILING.md) | Rust profiling: `profile_hot_path`, samply, flamegraph, parallel vs sequential. |
| [julia-and-blas-on-macos.md](julia-and-blas-on-macos.md) | Julia REPL (`using LinearAlgebra`, `BLAS.get_config`), `@code_native` / `@code_llvm`, profiling; macOS OpenBLAS vs Cargo `blas` feature; Accelerate expectations; [`../scripts/inspect_codegen_julia.jl`](../scripts/inspect_codegen_julia.jl). |
| Scripts ([`../scripts/`](../scripts/)) | [`inspect_codegen_julia.jl`](../scripts/inspect_codegen_julia.jl), [`inspect_codegen_rust.sh`](../scripts/inspect_codegen_rust.sh) — dump codegen for SIMD / BLAS comparison. |

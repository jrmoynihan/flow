...
- Avoid marketing-style “inspired by / derived from Tool X” wording in comments, docs, commits, and user-facing copy; describe behavior in neutral technical terms. Do credit academic papers and authors properly for algorithm reimplementations (citations in crate docs and READMEs)—do not strip scholarly attribution, invent author lists, or drop primary authors (AutoSpectral bioRxiv: Oliver Burton and co-authors). Reimplement the published default pipeline, not a reduced subset that omits core steps. Performance write-ups should use full prose (not jargon fragments or buzzwords), scaffold problem → solution → what changes in operation, point at the code, define or link concepts, and distinguish biological events from matrix entries (write “10k events”, not “10k” or “cells”). Apply that same prose standard to sibling crate `PERF_*` notes, not only the crate being edited.
...
- Keep filling realistic n×d performance matrices so callers can eventually auto-select the best method for a workload; scale CPU vs GPU benches enough to show behavior under pressure. Compare measured throughput against `docs/dev/PERF_LATENCIES.md` / `docs/dev/PERF_GAP.md`; if a hot path is far from that napkin math, retry `docs/dev/PERF_STRATEGIES.md` before inventing a new method.
...
- For PeacoQC and AutoSpectral Rust vs R speed claims, treat QC-core (post-load) as the headline metric and end-to-end as secondary context; CPU vs R is the publishable claim (include single-thread vs multi-thread when useful); prefer a reusable harness that can produce publishable numbers; document R↔Rust result agreement alongside timings; when GPU underperforms CPU across cases, do not headline GPU—recommend against that feature until improved.
...
- `flow-knn` is the shared KNN/ANN crate: self-query `compute_knn` → `KnnGraph` (PaCMAP, PARC); query-vs-library `AnnIndex` (Exact, usearch, ann-search-rs; GPU Exact/IVF behind `gpu`; GpuNnDescent AnnIndex is unimplemented because `query_nndescent_index_gpu` needs `&mut`) for stained-vs-library matching (AutoSpectral). GPU query pads features to ann-search-rs `LINE_SIZE`. `flow-pacmap` uses `fit_transform(Option<&KnnGraph>)` so callers can share one graph across embeddings. Shared PCA lives in workspace member `flow-dimensional-reduction`. Generic synthetic FCS lives in `flow_fcs::synthetic` (Cargo feature `synthetic`).
- `flow-clustering` optional `parc` feature implements PARC (HNSW → local/Jaccard prune → Leiden) via `Parc::fit` / `fit_with_knn`. `flow-autospectral` owns multi-AF discovery/matching and AutoSpectral v1.6 joint per-cell unmix (`discover_spectral_variants`, `unmix_autospectral_joint` / `JointUnmixConfig`; defaults follow R `unmix.fcs`, including `n_passes = 1` not the C++ signature default of 2) for OLS/TRU-OLS callers and is not gated on FlowSOM. Joint inner LS uses faer (explicit SIMD via pulp; custom per-event loops should use contiguous column-major slices, not scatter indexing). Rust-vs-R (AutoSpectral / AutoSpectralRcpp) uses `examples/compare_with_r` + R companion (`docs/comparison-with-r.md`); QC-core is the headline and published timings should include both single-thread and multi-thread for Rust and R/Rcpp. Criterion grids use `FLOW_AUTOSPECTRAL_BENCH_MATRIX` / `_LARGE`; `FLOW_AUTOSPECTRAL_FORCE_SEQUENTIAL=1` disables event Rayon.
...
- Performance-guided development: napkin primitive costs (CPU, cache/RAM/disk, GPU launch, alloc, type-width/packing, structure complexity) live in `docs/dev/PERF_LATENCIES.md`. This-host snapshot is `docs/dev/PERF_HOST.md` (`cargo run -p flow-perf-calibrate --release --example snapshot_host`). Compare Criterion to `docs/dev/PERF_GAP.md` before hunting. Retry a named keep from `docs/dev/PERF_STRATEGIES.md` before inventing a new optimization. Protocol: `docs/dev/PERF_PGD.md`. A ≥10× miss vs the *right* napkin (or an encoding that would change SIMD width / cache class on a primary size) is a Beads child of epic `flow-crates-0ap`, not a 5% `unsafe` A/B.

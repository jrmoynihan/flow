...
use flow_tru_ols::{
    DEFAULT_AF_CHANNEL_NAME, TruOlsUnmixing, UNMIXED_METHOD_TRU_OLS, UnmixingStrategy,
...
fn process_single_stained_file(
    ...
    let unstained_path_final =
        resolve_unstained_control_path(unstained_path, controls_dir, single_stain_controls_dir)?;
...
pub fn create_mixing_matrix_from_single_stains(
    ...
    let control_files: Vec<(String, PathBuf)> = if let Some(assignments) = control_assignments {
        ...
        info!("Using {} interactive control file assignment(s)", v.len());
        v
    } else {
        let entries = fs::read_dir(controls_dir)
            .with_context(|| format!("Failed to read directory: {}", controls_dir.display()))?;
            ...
            Ok(report) => {
                if want_unstained_snapshots {
                    ...
                    if let Err(e) = generate_control_cleanup_debug_plots(
                        ...
                        warn!(
                            "Failed to generate debug plots for unstained control: {}",
                            e
                        );
                    }
                }
...
fn generate_af_medians_plot(
    ...
    let path = control_plot_dir.join(format!("08_af_medians_raw_{}.jpg", endmember_name));
    std::fs::write(&path, bytes)?;
    info!("  ✓ Saved: {}", path.display());
    Ok(())
}
...
mod tests {
    ...
    fn test_endmember_display_label_keeps_dye_numbers() {
        let d = endmember_display_label(
            "Reference Group_A2 CD45 Spark UV 387 (Beads)_2026_03_05_11_55_59",
        );
